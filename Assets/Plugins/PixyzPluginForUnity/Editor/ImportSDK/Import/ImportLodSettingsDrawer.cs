...
namespace Pixyz.ImportSDK
{
    ...
    public class ImportLODSettingsDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (count > 0)
            {
                ...
                if (!locked && isLodSelected)
                {
                    ...
                    string[] options = FancyNames.Skip(selectedLodIndex).Take(FancyNames.Length - count + selectedLodIndex + 1).ToArray();//FancyNames.Slice(selectedLodIndex, FancyNames.Length - count + selectedLodIndex + 1);
