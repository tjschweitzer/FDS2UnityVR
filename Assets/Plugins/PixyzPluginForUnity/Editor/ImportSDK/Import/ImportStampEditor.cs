        ...
        private void drawAsSceneInstance(EventType eventType) {
            ...
            if (importStamp.importSettings) {
                foldout = EditorGUIExtensions.DrawCategory(ImportSettingsEditor.Style, "Settings Preset", true, () => {
                    string ext = importStamp.path.Substring(importStamp.path.LastIndexOf('.') + 1).ToLower();
                    bool lodPlacementDisabled = (ext == "e57" || ext == "pts" || ext == "ptx" || ext == "xyz" || ext == "rcp");
                    EditorGUI.BeginDisabledGroup(lodPlacementDisabled);
